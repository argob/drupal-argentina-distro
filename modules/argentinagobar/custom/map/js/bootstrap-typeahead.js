...
!function(jQuery){
  ...
  * ================================= */
  ...
  var Typeahead = function (element, options) {
    ...
    if (this.options.target) this.$target = jQuery(this.options.target)
  ...
  , replaceSelect: function () {
      ...
      this.$element = jQuery('<input type="text" />')
  ...
  , process: function (items) {
      return jQuery.isArray(items) ? this.processArray(items) : this.processObject(items)
    }
  ...
  , processObject: function (itemsIn) {
      ...
      jQuery.each(itemsIn, function (key, item) {
  ...
  , searchAjax: function (query, process) {
      ...
      this.ajaxTimeout = setTimeout(function () {
        ...
        jQuery.get(that.url, {'q': query, 'limit': that.options.items }, function (items) {
  ...
  , sorter: function (items) {
      return jQuery.isArray(items) ? this.sortArray(items) : this.sortObject(items)  
    }
  ...
  , render: function (items) {
      ...
      jQuery.map(items, function (item, value) {
        if (list.length >= that.options.items) return
        
        var li
          , a
        
        if (jQuery.isArray(items)) value = item
        
        li = jQuery(that.options.item)
  ...
  , next: function (event) {
      ...
      if (!next.length) {
        next = jQuery(this.$menu.find('li:not(.dropdown-header)')[0])
      }
  ...
  , listen: function () {
      ...
      jQuery(window).on('unload', jQuery.proxy(this.destroyReplacement, this))
    }
