...
class ConsultaApostillaFR extends Consulta {
  ...
  function get_form($form, &$form_state, $consulta) {
    ...
    //LENGUAJE
    //$path = $base_url.'/'.'legalizacion-internacional';
    ...
    //CAMPO 1
    //con este campo realizo la busqueda en la API de infoleg
    $anio = new FieldTextfield(' Année ');
    $anio->set_required(true);
    $form['wrapper']['anio'] = $anio->render();
    $form['wrapper']['anio']['#rules'] =
      array(
        array('rule' => 'numeric', 'error' => 'Le champ Année ne prend en charge que les chiffres'),
        array('rule' => 'length[4]', 'error' => 'La longueur entrée dans le champ Année est incorrecte'),
        array('rule' => 'notOnlySpaces', 'error' => 'Le champ Année non supporte les espaces'),
      );
    ...
    //CAMPO 2
    $numero = new FieldTextfield(' Numéro ');
    $numero->set_required(true);
    $form['wrapper']['numero'] = $numero->render();
    $form['wrapper']['numero']['#rules'] =
      array(
        array('rule' => 'numeric', 'error' => 'Le champ Numéro ne prend en charge que les chiffres'),
        array('rule' => 'length[8, 10]', 'error' => 'La longueur entrée dans le champ Numéro est incorrecte'),
        array('rule' => 'notOnlySpaces', 'error' => 'Le champ Numéro non supporte les espaces'),
      );
    ...
    //CAMPO 3
    $codigoreparticion = new FieldTextfield(' Code de dépendance ');
    ...
    // $form['wrapper']['codigoreparticion']['#value'] = 'DCT#MRE';
    ...
    //CAMPO 4
    $numeroorden = new FieldTextfield(' N° de Orden | N° de commande (champ 8 de l\'Apostille) ');
    $numeroorden->set_required(true);
    $form['wrapper2']['numeroorden'] = $numeroorden->render();
    $form['wrapper2']['numeroorden']['#rules'] =
      array(
        array('rule' => 'length[2, 20]', 'error' => 'La longueur entrée dans le champ  N° de commande est incorrecte'),
        array('rule' => 'notOnlySpaces','error' => 'Le champ N° de commande non supporte les espaces'),
      );
    ...
    $submitButton->setValue("Faire une requête");
  ...
  function get_response($request) {
    ...
    if ($request->code == 400 || $request->code == 404) {
      $response = theme('apostillafr_error');
...
//print_r($numordenapi);
      ...
      if ($arrayapi == $arrayuser && $numordenuser == $numordenapi ) {
        ...
        //var_dump($numordenapi);
        //die("el id:".$id);
        $consultaPdf = new ConsultaApostillaPdfFr(ApiGatewayAuth::getInstance(), ['numerodocumento' => $id]);
