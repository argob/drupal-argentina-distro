...
#namespace Drupal\argentinagobar_servicios;
  ...
  class TramiteTransferObject extends TransferObject{
    ...
    function get_modalidad($node_wrapper){
      ...
      $this->modalidad_digital    = $node_wrapper->field_modalidad_digital->value();
      $this->modalidad_telefonico = $node_wrapper->field_modalidad_telefonico->value();
      $this->modalidad_presencial = $node_wrapper->field_modalidad_presencial->value();
      $this->modalidad_otro       = $node_wrapper->field_modalidad_otro->value();
      $this->modalidad_otro_especificar = $node_wrapper->field_modalidad_otro_especificar->value();
    }
    ...
    function get_oficina($node_wrapper){
      $this->oficina = $node_wrapper->field_oficina->value();
      $this->oficina_otra = $node_wrapper->field_oficina_otra->value();;
    }
    ...
    function get_alias($node_wrapper){
      $this->alias = url('node/' . $node_wrapper->getIdentifier(), array('absolute' => FALSE));
    }
    ...
    function get_servicios_relacionados($node_wrapper){
      ...
      foreach($node_wrapper->field_servicios_relacionados->getIterator() as $servicio){
        ...
        $servicios['items'][] = array(
          ...
          'alias' => url('node/' . $servicio->field_servicio->getIdentifier(), array('absolute' => FALSE)),
